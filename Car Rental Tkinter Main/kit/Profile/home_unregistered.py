...
from tkinter import Tk, Canvas, Entry, Text, Button, PhotoImage, Frame, Scrollbar, VERTICAL, Label
...
def main_window():
    ...
    def on_enter_login(e):
        login.config(bg="#90EE90")  # Light green hover color

    def on_leave_login(e):
        login.config(bg="#006400")  # Dark green default color

    # Function to change button color on hover for Register
    def on_enter_register(e):
        register_btn.config(bg="#90EE90")  # Light green hover color

    def on_leave_register(e):
        register_btn.config(bg="#006400")  # Dark green default color
    ...
    canvas.create_text(649.0, 476.0, anchor="nw", text="Car Types", fill="#000000", font=("Inter ExtraBold", 28 * -1))

    header = PhotoImage(file=relative_to_assets("image_4.png"))
    h = canvas.create_image(960.0, 37.0, image=header)

    canvas.create_text(220.0, 20.0, anchor="nw", text="Car Rental System", fill="#FFFFFF", font=("Helvetica", 25, "bold"))
    ...
    register_btn.place(x=1570.0, y=17.5, width=130.0, height=43)
    ...
    login.place(x=1420.0, y=17.5, width=130.0, height=43)
    login.bind("<Enter>", on_enter_login)
    login.bind("<Leave>", on_leave_login)

    def goto_login():
        window.destroy()

        login = importlib.import_module("chen.buildregister.login")
        login.main_window()
